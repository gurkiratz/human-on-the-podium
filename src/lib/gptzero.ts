import { gptZeroKey } from "./env";
import { isThin } from "./chunker";
import type { ClassProbs, Detection, ScoredSentence, Verdict } from "./types";

export { SENTENCE_AI_THRESHOLD } from "./constants";
...
/**
 * Scores one chunk of transcript.
 *
 * Sentence heat-map uses generated_prob / sentence class_probabilities.ai
 * (highlight_sentence_for_ai is unreliable per GPT_ZERO_FINDINGS.md).
 */
export async function scoreChunk(
  ...
  const sentences: ScoredSentence[] = (doc.sentences ?? []).map((s) => ({
    sentence: s.sentence ?? "",
